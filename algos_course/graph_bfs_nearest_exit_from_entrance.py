...
[entrancerow, entrancecol] denotes the row and column of 
...
class Solution:
    def nearestExit(self, maze: list[list[str]], entrance: list[int]) -> int:
        ...
        def exitMaze(row, col, level):
            return (row == 0 or row == m - 1 or col == 0 or col == n - 1) and level > 0

        while queue:
            r, c, level = queue.popleft()
            if exitMaze(r, c, level):
                return level
            for dx, dy in directions:
                if valid(r + dx, c + dy) and (r + dx, c + dy) not in seen:
                    seen.add((r + dx, c + dy))
                    queue.append((r + dx, c + dy, level + 1))
