...
class Solution:
    def splitArray(self, nums: list[int], k: int) -> int:
        ...
        def subs_reqd(max_sum_allowed: int) -> int:
            current_sum = 0
            splits_allowed = 0
            for x in nums:
                if current_sum + x <= max_sum_allowed:
                    current_sum += x
                else:
                    current_sum = x
                    splits_allowed += 1
            return splits_allowed + 1

        l = max(nums)
        r = sum(nums)
        while l <= r:
            m = (l + r) // 2
            subs_given_m = subs_reqd(m)
            if subs_given_m <= k: # means mid is too big
                r = m - 1
                ans = m
            else:
                l = m + 1
